...
function ConfirmarMatricula() {
  ...
  const [formDataState, setFormDataState] = useState(location.state || {});
  const [searchParams, setSearchParams] = useState({
    ano_execucao: formDataState.ano_execucao || new Date().getFullYear(),
    nome_programa: formDataState.nome_programa || "",
    nome_curso: formDataState.nome_curso || "",
    acao: formDataState.acao || "",
    id_curso: formDataState.id_curso || 0,
    gerar: "",
  });
  useEffect(() => {
    if (formDataState && formDataState.id_curso) {
  ...
  const handleChange = (e) => {
    ...
    setSearchParams((prev) => {
      ...
      if (name === "nome_curso") {
        const cursoSelecionado = dtcursos.find(
          (curso) => curso.id.toString() === value
        );
        if (cursoSelecionado) {
          updatedParams = {
            ...updatedParams,
  ...
  const handleLimpar = async () => {
    ...
    // Resetar formDataState também
    setFormDataState({});

    // Resetar searchParams com valores padrão
    setSearchParams({
      nome_formando: "",
      ano_execucao: new Date().getFullYear(),
      nome_programa: "",
    ...
    candidatos.forEach((item) => {
      ...
      if (temCursoSelecionado) {
        ...
        const statusReal =
          ...
          item.cursos_inscritos.find((c) => ["1", "2"].includes(c.opcao))
            ?.status_matricula;
    ...
    <>
      <div className="container-fluid p-2">
        <Row
          md={12}
          xs={12}
          className="bg-white shadow rounded p-1 mb-1 align-items-center"
        >
          {filtros ? (
            <>
              <Row
                md={12}
                className="d-flex flex-wrap align-items-center mb-2 mt-3 p-2 "
              >
                <Col md={3} className="">
                  <SelectField
              ...
              <Col md={12}>
                <Row md={12} xs={12} className="mt-0 g-2">
                  ...
                  >
                    ...
                    >
                      Voltar
                      <FontAwesomeIcon icon={faUndo} className="ms-2" />
                    </Button>
                  </Col>
                </Row>
              </Col>
