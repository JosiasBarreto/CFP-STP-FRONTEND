...
function TablePrograma({ carregarPrograma, formik, deletarPrograma, datas, isLoading, isFetching }) {
                    ...
                    {currentItems.length > 0 ? (
                        currentItems.map((user, index) => (
                            <tr key={index}>
                                ...
                                <td>{user.duracao}</td>
                                <td>{user.publico_alvo}</td>
                                <td>{user.data_criacao}</td>
                                <td>
                                    <Button
                                        variant='outline-success'
                                        onClick={() => carregarPrograma(formik, user.id, user.nome, user.descricao,user.duracao,user.publico_alvo )}
                                    >
                                        <BsPencilSquare /> Editar
                                    </Button>
                                </td>
                                <td>
                                    <Button variant='outline-danger' onClick={() => deletarPrograma(user.id)}>
                                        <BsTrash /> Eliminar
                                    </Button>
                                </td>
                            </tr>
